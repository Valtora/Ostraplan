...
**Status:** SCOPE LOCKED (2026-07-05) — all decisions settled (§1, §10). **Phases 1–2 SHIPPED 2026-07-06.** Phase 1 (identity + context + diff): `Placement.OriginStrID`, `ShipDocument.SourceSave`, `SaveEditImport.ImportForEditing` → full `SaveShipContext`, `.oplan` persistence, pure `ShipDiff`. Phase 2 (inject to a copy): `SaveEdit.BuildInjectedShip` (diff-driven `aItems`/`aCOs` rebuild, original `vShipPos` frame, grid grow-not-shrink, recomputed rooms/rating, hard validation) + `SaveEdit.WriteCopy` (folder copy, zip rename, `saveInfo` update, splice `ships/<RegID>.json`) + the "Update Ship in Save…" UI (cargo-loss warn+confirm, overwrite-after-confirm, change report) + on-demand context re-location for a reopened `.oplan`. 128 tests green; verified against a real save (no-op inject round-trips every item; original byte-untouched). **First in-game E2E (2026-07-06) surfaced + fixed the biggest bug:** new parts (added *or* re-skinned) loaded with `"missing save data ... Skipping"` and vanished — the save-load path needs a CO per item, so §2 discovery #2 was corrected and `SaveEdit` now synthesizes a pristine CO per new part (`aConds:["DEFAULT"]`) with a `Validate` guard. **Phase 3** (harden: moved-container-cargo & crew-`nDestTile` edge cases, occupied-deleted-floor semantics, re-run the in-game E2E) is next — owner-driven.
...
2. **A new item needs a synthesized *pristine* CO** — ⚠ **CORRECTED 2026-07-06** (the original premise was wrong and caused a catastrophic in-game failure). The default-CO fallback (`GetCondOwner` → build from the def) fires **only for a *template* load** (`bTemplateOnly == true`). Loading a real **save** (`DataHandler.SpawnItems`, `bTemplateOnly == false`) does the opposite: any `aItems` entry whose `strID` isn't in `dictCOSaves` is logged (`"Trying to load a CO ... with missing save data ... Skipping"`) and **dropped**. So every new part needs a matching `aCOs` entry. The fix stays cheap: synthesize a CO of `{ strID (= the item's), strCODef (= the def), aConds: ["DEFAULT"] }` — `CondOwner.SetData` treats `"DEFAULT"` by repopulating the def's own `aStartingConds` (cooverlay skins resolve to their base via `GetCondOwnerDef`), so the part loads freshly-built. `SaveEdit.Validate` now hard-aborts if any item lacks a CO, the exact invariant the game enforces.
...
| **New** (added in Ostraplan) | no `OriginStrID` | fresh `aItems` entry (fresh `strID`, def) **+ a synthesized pristine `aCOs`** (`strCODef` = def, `aConds: ["DEFAULT"]`) — a save load skips any item lacking a CO (see §2, corrected) |
...
4. **Rebuild `aCOs`**: keep every CO whose `strID` still exists after the diff (kept/moved parts, **all** non-structural COs — crew, loot-spawners — and surviving cargo); drop the rest; **and add a synthesized pristine CO for every new part** (§2, corrected — a save load skips an item with no CO).
