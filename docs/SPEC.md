...
**Status:** P0 foundation + P1 placement law + **P2 Law milestone** (rooms/airtightness, certification, Ship Rating + law report) + **P3 Interop** (export as spawnable mod · template import · save-game import) shipped (2026-07-05). **P4 QoL** shipped 2026-07-05: flood-select, Replace, big-ship performance, PNG snapshot, the **cooverlay theme picker** and the **bill of materials**. Two backlog items were **dropped**: per-mod palette toggles (managing mods is out of Ostraplan's scope) and power/O₂ budgets (per-device draw and gas rates aren't in the data — an honest balance would need network simulation, a non-goal) (§11). **P5 Save-Edit Phases 1–2** shipped 2026-07-06: import your live ship for editing, and write the edited structure back into a **copy** of the save (crew/cargo/identity preserved, original untouched) — engine + "Update Ship in Save…" UI; only Phase 3 hardening + in-game E2E remain (§8.5, §11). **Light Viz** (v0.46) adds the game's interior lighting simulated live on the canvas — multiplicative shadow-cast reveal (`final = sprite × light`) with **Brightness** / **Unlit black** controls; interior only, exterior star light deferred (§7).
...
- **Light Viz** (shipped v0.46, `ShipCanvas.ShowLight`, View ▸ **Light overlay** or **`L`**): the game's interior lighting simulated live on the plan, so lighting fixtures can be placed and judged before building. `LightNetwork.Build` casts, per installed light, a **visibility polygon** — a ray-fan from the light's exact sub-tile position that stops at walls (`IsWall` only; fixtures don't occlude, matching the game's smooth room lighting) — in document coords. The canvas renders it **multiplicatively** like the game (`final = sprite × light`): the ship is revealed through a brightness opacity mask over black, bright where a light reaches and dark where none does, with a faint colour tint. Two persisted controls on the **Light Viz** submenu (slider + numeric entry): **Brightness** (reveal gain — how strongly a light lifts its area) and **Unlit black** (the ambient floor: 0 = the ship stays full-bright, a glow for editing; 1 = unlit areas go black, the in-game look). Computed off-thread in the debounced scan only while on, like PowerViz/RoomViz; it is lighting *visualisation* (a preview — there is no darkness gameplay stat), not a photometric simulation. **Interior only** in this release: exterior sun/star light and glass transmission (modelled in the game) are not yet ported. Model + verified constants: [GAME-INTERNALS §6](GAME-INTERNALS.md).
