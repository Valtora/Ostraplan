...
**Version:** 0.7 · updated 2026-07-18
**Status:** P0 foundation + P1 placement law + **P2 Law milestone** (rooms/airtightness, certification, Ship Rating + law report) + **P3 Interop** (export as spawnable mod · template import · save-game import) shipped (2026-07-05). **P4 QoL** shipped 2026-07-05: flood-select, Replace, big-ship performance, PNG snapshot, the **cooverlay theme picker** and the **bill of materials**. Two backlog items were **dropped**: per-mod palette toggles (managing mods is out of Ostraplan's scope) and power/O₂ budgets (per-device draw and gas rates aren't in the data — an honest balance would need network simulation, a non-goal) (§11). **P5 Save-Edit Phases 1–2** shipped 2026-07-06: import your live ship for editing, and write the edited structure back into a **copy** of the save (crew/cargo/identity preserved, original untouched) — engine + "Update Ship in Save…" UI; only Phase 3 hardening + in-game E2E remain (§8.5, §11). **Light Viz** (v0.46, rewritten **pixel-exact** in v0.47) simulates the game's lighting live on the canvas — the full deferred light pass ported from the decompiled DLL and disassembled shaders (real `aShadowBoxes` occluders, exact falloff, normal-map relief, screen-blend stacking, glow decals) plus optional parallax **exterior daylight** (§7).
...
- **RoomViz** (shipped v0.45, `ShipCanvas.ShowRooms`, View ▸ **Rooms** or **`C`** — the game's own term is Compartment): the §6.5/§6.7 partition and certification surfaced live on the canvas rather than only in the law report. `RoomOverlay.Build` runs the same `RoomBuilder` + `RoomCertifier` pass and yields, per **interior** compartment, its document-space tiles, certified spec, tile count, room value and — when it certifies as nothing — the `ShipAnalysis.NearMisses` lines (what to add; which member part **forbids** it). Each room tints in its own palette hue (shared with the rating snapshot, so the two read alike), unsealed rooms take the snapshot's hazard red, and labels suppress below a zoom threshold. The exterior void is excluded by design: its fill reaches the grid edge so it would tint the whole plan, and the interesting case — a compartment open to space — reads as an interior room *losing* its tint (leak tiles have their own visualisation, §6.9). Computed off-thread in the debounced scan only while the overlay is on, exactly like PowerViz.
  - **Rendering**: the per-room fills bake into one frozen pan-zero `Geometry` each and the labels' `FormattedText` is built once per overlay — a rectangle per tile plus a text layout per room, every frame, made a station unpannable. Both rebuild only on an overlay/zoom change (`_roomGeoDirty`), mirroring PowerViz's `_powerGeoDirty`.
  - **Labels** counter-rotate about their own anchor so they stay upright under view rotation (as the connector badges do), and are laid out **biggest room first**, each taking the first free spot of: full block on the centroid → nudged above → nudged below → title only → dropped. Collisions resolve in screen space (a counter-rotated label is an upright box centred where its anchor lands, and a `(0,dy)` nudge survives the round trip), and off-screen labels are skipped.
- **Light Viz** (shipped v0.46; **pixel-exact rewrite v0.47**, `ShipCanvas.ShowLight`, View ▸ **Light overlay** or **`L`**): the game's lighting reproduced exactly on the plan. The pipeline is a full port of the game's deferred light pass, taken from the decompiled `Visibility`/`Occluder`/`Item` and the **disassembled `Sprites/LoSPass` GPU shader**: occlusion from the item defs' `aShadowBoxes` (windows are glass and pass light; thin/aero walls don't occlude; open doors spill light; beds/canisters/reactor pods DO shadow while being lit themselves), wall faces lit by the skirt extrusion, the exact falloff curve (`F=3`, `Z=0.25`, `atten = 1/(F²(d²+Z²)+0.1)`), per-pixel normal-map relief (`strImgNorm`), screen-blend light stacking, and additive lamp glow decals. Rendered in software at the game's native 16 px/tile (`VisibilityMesh` + `LightComposite` in Core, unit-tested against the shader curve): the canvas bakes doc-space albedo + normal bitmaps, the compositor runs off-thread in the debounced scan, and one frozen bitmap draws under the overlays — unlit hull is a black silhouette, the in-game truth (the game never draws the sprite layer; the visible ship IS the light sum). No brightness/dimming tuners (the old v0.46 sliders are gone — the output is game-exact). **Exterior daylight** (View ▸ Light Viz): pick a parallax location + sun angle and its real radius-1000 sun lights shine on the design, hull-occluded, streaming through glass. Still a *preview*, not a Law port (no darkness gameplay stat). Model + verified constants: [GAME-INTERNALS §6](GAME-INTERNALS.md).
...
- **Documents**: New / Open / Save / Save As / Recent; **template browser** for "start from a Vagabond" (core + modded ships, rendered previews). A design opened while a mod it depends on isn't loaded is held **read-only** — the missing parts and their mods are named on open and the chrome shows a standing "MISSING MODS" warning — because saving rewrites the design as it stands and would drop those parts for good, and building over where they belong yields a ship that's invalid in-game. The hold is a question, not a verdict (v0.45): enabling the mods and reopening brings the parts back, **or** the user may confirm the drop on Save, which is the right answer when they have deliberately moved off that mod. Confirming clears `_unresolvedParts` and lifts the hold — refusing outright left no way to say "I'm done with those parts" short of hand-editing the file. See §8.1.
...
- **Write-back** (`SaveEdit.BuildInjectedShip` → `WriteCopy`, the "Update Ship in Save…" action) diffs the edited layout against the retained originals (kept / moved / new / deleted — `ShipDiff`), then rebuilds the ship record: kept parts verbatim, moved parts repositioned with their cargo (in the original `vShipPos` frame), new parts as fresh entries (a pristine CO synthesized per new part so a save load doesn't skip them), deleted parts and their cargo dropped; `aCOs` filtered, `aRooms`/`aRating` and the grid recomputed. It hard-validates, then duplicates the save folder (zip renamed, `saveInfo` updated) and splices only `ships/<RegID>.json`. The **default is a copy**, leaving the original untouched; **overwriting in place** is an explicit opt-in that first copies the whole save to a separate, loadable **backup save in the Saves folder** — beside the save, never inside it, so deleting a broken edit can't take its backup with it — then edits the original. A **Wear** slider (§6.12) can additionally re-roll the condition of **every** installed part to a chosen average (default on, at the vanilla ~88%), overriding existing per-part damage; unticked, each kept part keeps its in-save wear and new parts stay pristine.
  - **The grid frame is the game's, not ours** (fixed 0.45.0): a loading ship *rebuilds* its tilemap as `bbox(item footprints) ± a one-tile margin` and only uses the saved `nCols`/`vShipPos` for the shallow view, so the inject must write exactly that frame — `aRooms`/`aZones` are flat `col + row·nCols` indices, and a frame of a different width or origin silently relocates every one of them (ghost rooms + skewed zones). It follows that the frame may **shrink**, so crew `nDestTile` is recomputed on any reframe. Asserted against every local save (`SaveEditFrameTests`).
  - **Room COs are dropped, not kept** (fixed 0.45.0): a room's saved `strID` *is* its backing `Compartment` CO's id, so regenerating `aRooms` with fresh ids while keeping the originals leaves an unbound IsRoom CO per room — a ghost room. The inject drops every room CO and lets `Ship.CreateRooms` rebuild each from the saved id. Room atmosphere is regenerated regardless (`bPrefill`). See GAME-INTERNALS §8.
  - **Unresolved (missing-mod) items and stand-ins** (v0.45, `Substitution`): an item whose def isn't in the loaded data is skipped by the import yet preserved by the write-back, so it is invisible to the rooms/frame/rating engines while still present in the save — a missing modded **wall** stops dividing a room, and a missing part at the hull edge under-sizes the frame. Both corrupt the ship on load, for the same reason a hand-built frame did. `Substitution.Outstanding` recomputes them from the save record (top-level items whose `strName` doesn't resolve and that no stand-in covers), so it is correct after an `.oplan` reopen. The user may put a real part in an item's place: it is placed at the original's world centre using the **stand-in's** footprint (there is no other footprint to use — the original's def is what's missing), tagged with the original's `OriginStrID`, and on inject the original item/CO (and anything parented to it, reported as a cargo loss) is dropped while the stand-in writes as a fresh part. What Ostraplan models is therefore exactly what lands in the save. **No new persisted state**: a stand-in is identifiable as a placement whose origin id names a real save item that is *not* one of the modelled `Origins`, a combination nothing else produces — so it round-trips through `.oplan` for free, survives a move, and deleting it reverts to leaving the modded item alone. Leaving items unresolved is allowed but gated by a hard confirm on write-back (§7).
...
- **Off-thread work** (`Ui.OffThread`, v0.43.2): all background work goes through it, never `Task.Run` directly. Two rules it exists to enforce:
  1. **Never capture a UI object in the lambda.** A WPF control belongs to its creating thread, so reading a dependency property off it throws "the calling thread cannot access this object because a different thread owns it" — and when the capture is an *argument* (`OffThread(() => Engine.Run(dlg.Wear))`), the throw lands before the callee runs at all, so the engine is never entered. Read the value on the UI thread and capture the local (the `env0`/`catalog0` convention). Debug builds check this by walking the lambda's closure and naming the offending capture; Release builds strip the check. A **frozen** `Freezable` is exempt: it is immutable and thread-safe, which is why `SpriteCache` can build thumbnails on the worker.
  2. **Don't let the document change under the engine.** The scan hands off a `ShipDocument.Snapshot` (grid-only, which is all it reads). Export, save-edit and rating also read cargo, zones, loose items and links, none of which Snapshot copies, so they take the live document and `MainWindow.FreezeDoc` freezes editing for the run instead. It disables `Chrome` — the entire editing surface — rather than picking off individual controls: gating only the canvas and the keyboard leaves the toolbar live, and **Undo** during a multi-second Ship Rating mutates the very list the analysis is walking. The gate (`FreezeGate`) is nestable like `ShipDocument.SuspendChanged`, so an export started during a rating thaws on the *last* scope out, not the first.
- **Exception discipline**: a caught exception must name a cause the user can act on (`InvalidDataException` = this ship can't be represented; `IOException`/`UnauthorizedAccessException` = the write didn't land). Everything else is our bug: let it reach `App.DispatcherUnhandledException`, which logs the stack to `error.log` and keeps the app alive. A blanket `catch (Exception)` behind a domain message ("The edit can't be written back") destroys the stack and misattributes our bugs to the user's data — that is exactly how the v0.43.1 thread-affinity bug read as a save problem.
