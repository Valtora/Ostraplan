...
- **Canvas**: place LMB (repeat-place while armed); rotate **`R`** (`Shift+R` reverse) — deliberately the same key as the game's build mode, and the general rule: **where the game has an equivalent binding, Ostraplan uses it**; **flip a selection `H`** (horizontal, left↔right) / `Shift+H` (vertical, up↔down); cancel `Esc`; delete `Del`/eraser mode; move by dragging a selection; box-select; copy/paste of selections. Ghost preview is green/red with failing cells highlighted and a tooltip naming the unmet requirement. Pan: Space-drag/MMB; zoom: wheel, integer pixel multiples (1×–8×, NearestNeighbor — crisp pixel art). Overlays: room fill tinted per room with spec label; seal overlay; advanced socket-debug overlay.
