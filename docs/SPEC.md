...
**Version:** 0.6 · updated 2026-07-07
...
**Ship identity.** The dialog also sets the in-game `publicName` and the `make`/`model`/`year`/`designation`/`description` flavor fields. `publicName` matters: decompiled `Ship.InitShip` re-rolls a random `GetShipName()` on **every** spawn unless the on-disk value is a real string (not `null`/`""`/`"$TEMPLATE"`), so a blank export loses the ship's identity each spawn — export writes the typed name through and guards the two sentinel values. `strRegID` (the transponder "callsign") is **not** controllable from a data mod — `StarSystem.SpawnShip` overwrites it with a freshly-minted ID before `InitShip` runs, always (two live ships can't share a RegID) — so the dialog doesn't expose it. `objSS` position is a **small nonzero** placeholder, never exact `(0,0)`: the loot-spawn path (kiosk/Special-Offer/starting-ship) doesn't reposition a template the way import does, and `(0,0)` around "Sol" is the star's own origin (the "spawns inside the sun" bug).
