...
| `reactor` | object / absent | What the designer set on a fusion core's own control panel: `bus` (`"Off"` / `"Batt"` / `"Chrg"`, the game's PWR BUS knob), `purge` (`"Off"` / `"Rough"` / `"Turbo"`, its CORE PURGE knob), `torch`, and the eight ignition-sequence switches `align`, `coilFwd`, `coilRear`, `cryo`, `fuelReg`, `ignition`, `mhd`, `pellet`, plus the two sliders `cycle` and `flow` (0-1). Absent for every part that is not a core and every core left cold, which is what a def ships as. The game's own reactor simulation reads these keys off the item every tick, so a core with `bus` on and `ignition` true spawns lit. Both knobs are stored as names rather than numbers so the file stays readable; an unrecognised one reads back as `Off`. Sliders are clamped to 0-1 on load. Additive since v1. |
| `weapon` | object / absent | What the designer set on this weapon's page of the Weapons MFD: `group` (the firing group **1-9**, as the nav console numbers it and as the fire keys are bound — the game stores it one lower, and only this field's reader knows that), `manual` (the weapon is left out of the auto-fire list and answers only to its group's key), and `target` (`"All"` / `"Ships"` / `"NonShips"`, a point-defence cannon's target select, which the game stores as two mutually exclusive flags). Absent for every part that is not a ship weapon and every weapon left at what its def ships with, which is nearly all of them — no shipped ship authors any of this. A group outside 1-9 is clamped on load and an unrecognised `target` reads as `All`, since the file is hand-editable and neither a group the game does not have nor a silently disarmed cannon is a thing to carry into an export. Additive since v1. |
