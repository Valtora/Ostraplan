...
### Lighting — Light Viz (`LightNetwork`, from `Visibility`/`JsonLight`/`Item`, 0.15.x)

- **The game's model.** A **2D deferred, per-light shadow-cast** render. Global ambient is **black** (`GameRenderer.clrAmbient`), so darkness is simply the absence of light. Each fixture attaches its item def's **`aLights`** (names into `data/lights`, the `JsonLight` DTO). `Item` creates a real `Visibility` light **only if `strColor != "Blank"`** (a `"Blank"` entry is a glow-sprite decal only, no illumination), at radius `fRadius` or, when it is ≤0, `Visibility.DEFAULTVISIBILITYRANGE` = **6** tiles. A colour's **alpha is its intensity** (`data/colors`, e.g. `WhiteLightCeiling` = 230,231,255,**100**); world scale is **16 px/tile** (`ptPos`/16); the shader falloff exponent `nLightFalloff` = **3** and light height `fLightZ` = 0.25 are cosmetic. Occlusion is real angular shadow-casting against wall segments (`Occluder` / `Visibility.AddOccludersFromCrewSimBlocks`), **skipping glass** (`block.bIsGlass`); exterior daylight is ordinary radius-1000 lights parented to a far "sun" (`ParallaxController` reads `data/parallax` `aSunLights`) and occluded by the hull. Lighting is **rendering only** — there is no darkness gameplay stat (no `LightLevel` / "too dark to work" cond), so it gates nothing and is a **preview, not a Law port**.
- **Ostraplan (`LightNetwork`, interior).** Each casting light is placed at its **exact sub-tile** position (`ShipGrid.MapPointPos`, so a wall light faces its room) and its lit region is a **visibility polygon**: a 180-ray fan marches the grid (a DDA) and stops at the exact point each ray enters a wall, giving smooth falloff + straight shadow edges. **Only `IsWall` occludes** — deliberately *not* `IsObstruction`: the game lights a room smoothly across its clutter, so blocking on every console/tank would cast a jagged shadow off each one (the faceted mess a per-obstruction test produces). The renderer is **multiplicative** (`final = sprite × light`, matching the game): the ship is revealed through a brightness **opacity mask** over black — an ambient floor (the user's "Unlit black") plus each light's radial reveal (scaled by the user's "Brightness" reveal gain) — with a faint colour tint on top. Computed off-thread in the debounced scan while the overlay is on (like PowerViz/RoomViz). **Interior only**: exterior sun/star light and glass transmission are modelled in the game but not (yet) ported. **Re-verify per patch:** ambient black, colour ≠ Blank casts, default radius 6, falloff 3, intensity = alpha/255, 16 px/tile.
