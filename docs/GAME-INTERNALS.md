...
- **Contained cargo needs `aCondOverrides` to survive a template spawn** — `Ship.SpawnItems` drops any parented item without `aCondOverrides`/`bForceLoad` and refills the container from def loot; export stamps a benign `StatDamage=0` marker (which also suppresses the root container's default loot) (§8).
