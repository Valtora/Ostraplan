...
Tabs, so several designs are open at once and you can copy between them. PNG and SVG snapshots. Light/dark theming. A **plan backdrop** you choose: a solid colour, a checkerboard, or one of the game's own places. Optional scale markings every 5, 10 or 20 tiles. A resizable palette and inspector that can be hidden altogether. **UI scaling from 80% to 200%**, up for a high-resolution monitor run at 100% Windows scaling and down to fit more into the window you have. And an optional background update check.
