...
4. **Editor + authoring** ✅ *(done — add/remove; 2026-07-07)* — add/remove loose cargo in the inventory window at any drill-in depth, undoable. Add-picker offers only what the container accepts (`ContainerFilter.AcceptedBy`) and drops the item into the first free cell, **blocked when the grid is full** (`InventoryGrid.FirstFreeCell` = capacity, "the Law"); identical stackable items stack (lead + members, up to `nStackLimit`); right-click removes one or the whole stack (a container's contents leave with it). Every edit is a `SetCargoCommand` on the command stack. **Inject reconciliation** (`SaveEdit.BuildInjectedShip`): current `Placement.Cargo` is the source of truth via a **keep-set** — an original contained item still present is kept verbatim, one no longer present is dropped (item + 1:1 CO), an authored item is synthesized (fresh item + pristine CO, `inventoryX/Y` from the grid cell); this generalizes and subsumes slice 1's re-skin transfer. Removing from a *kept* container fires no false "cargo will be deleted" warning (that stays scoped to deleted containers). `.oplan` persists a **full cargo snapshot** per edited container (authoritative on reopen; un-edited containers still re-read from the save). Equipped slots stay read-only. New: `CargoEdit`, `CargoItem.Authored`, `ShipDocument.SetCargo`/`IsCargoEdited`, `SetCargoCommand`, `OplanCargo`, `AddCargoDialog`. **Owner GUI review + in-game E2E pending.** Pre-loading containers in *exported* ships (Phase 3 slice 2) still deferred.
