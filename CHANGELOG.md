...
- **Simulate: fire a micrometeoroid or a weapon at the design and see what breaks.** A new
  **Simulate ▾** menu with **Micrometeoroid Strike…** and **Weapon Impact…**. **Drag a line across
  ...
  it damages is tinted green through amber to red by how much of its life is left, outlined so a
  light scratch is still easy to spot, and repeated strikes build up so you can see what a bad run
  does. Damage is never saved and never reaches your `.oplan` — **Start over** puts the ship back
  to pristine.
...
- **A `.oplan` no longer belongs to a save game.** A design imported for editing used to record the
  ...
  of any of its containers. A design now carries everything it describes, container contents included,
  and names no save at all. Delete the save it came from, move the file, or hand it to somebody who
  has never seen your playthrough, and it still opens, edits, prices and exports exactly as it did.
  - **Which ship a write-back goes over is asked at the write.** A design you imported this sitting
    still offers the ship it came from, so the import-edit-write round trip is unchanged. Reopen it
    another day and **Update Ship in Save** asks which save and which ship, the same as it does for a
    design you drew from scratch, and writing it back onto the ship it came from is one of the answers.
  - **A write-back re-reads each container from the ship it is about to write over**, so a locker you
    rearranged in game after saving the design is not reverted by it. Only containers you edited
    yourself in the inventory editor are held as the design's own and left alone. This used to happen
    on open, against the save the file named, which is why the file had to name one.
  - Designs written by 0.92.x and earlier still carry that save. Opening one reads its container
    contents back from that save once, as before, and saving it writes them into the design and drops
...
### Fixed
- **EVA suit slots now visible and act as a nested container.** Previously, putting one in a
  container and writing the design to your save gave you a suit with no battery, O2 or filter
  compartment at all: nowhere to put anything, and no way to power it. A suit's four compartments
  are *slotted* onto it, the way a helmet slots onto a head, and Ostraplan was writing them as
  ordinary cargo sitting inside it. A suit is not a container in the first place, so there was
  nothing for them to sit in and the game attached none of them. The same was true of a backpack's
  four pouches, a coverall's two hip pockets and a wrist PDA's data store; a backpack looked fine
  only because its own 4×4 grid works either way. Pockets now go into their slots, one each, and
  reach your save that way. A design you already saved is corrected when you open it, and so is a
  ship imported back out of a save that an earlier version wrote, so writing it out again repairs it.
