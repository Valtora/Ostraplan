...
public sealed class NewShipDriver : ExportDriver
{
    ...
    public override string Blurb =>
        "Adds the design to a copy of a save as a brand-new ship you own, parked a few kilometres out and reachable " +
        "by P.A.S.S. ferry. Your original save is never modified.";
    ...
    public override async Task<BuildOutcome> BuildAsync(WizardSession session)
    {
        ...
        var facts = new List<ReviewFact>
        {
            ...
            new("Writes to", $"a new save named {Path.GetFileName(_outDir)}"),
            new("Original save", $"\"{ctx.SaveName}\" is not modified"),
        };

        return new BuildOutcome(facts, _report.Warnings, []);
    }
    ...
    public override async Task<DoneReport> WriteAsync(WizardSession session)
    {
        ...
        var plan = session.Plan;
        var price = plan.NewShip.Charge ? plan.NewShip.Price : 0;
        var (outDir, report) = await WriteOffThread(ctx, _regId, ship, built, price, _outDir);

        // the context has now claimed the ship and taken the deduction, so it cannot be written again
        _ctx = null;
        _ship = null;
        SaveSettings(session);
        AuditLog.Add($"Granted ship \"{plan.ShipName}\" ({report.RegId}) into {outDir}.");
    ...
    private static Task<(string OutputDir, GrantReport Report)> WriteOffThread(
        ...
        Ui.OffThread(() => SaveGrant.WriteGrant(ctx, regId, ship, report, price, outDir));
